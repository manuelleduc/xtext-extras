...
import org.eclipse.xtext.xtext.generator.mbase.MbaseGeneratorFragment2;
 ...
 */
final class GenerateMbase {
	/**
	 * Can't use MWE2 because of circular dependencies
	 */
	public static void main(String[] args) {
		final String root = "..";
		final String projectName = "org.eclipse.xtext.mbase";
		final String runtimeProject = root + "/" + projectName;
		final String uiProject = System.getProperty("uiProject", root + "/../xtext-eclipse-xp/" + projectName + ".ui");	
		...
		final String fileHeader = "/*******************************************************************************\n" +
			...
			" *******************************************************************************/";
		final String mbaseGenModel = "platform:/resource/" + projectName + "/model/Mbase.genmodel";
		...
		final XtextGenerator generator = new XtextGenerator() {{
			...
			addLanguage(new XtextGeneratorLanguage() {{
				...
				setGrammarUri("classpath:/org/eclipse/xtext/mbase/Xtype.xtext");
				addReferencedResource(mbaseGenModel);
			...
			addLanguage(new XtextGeneratorLanguage() {{
				String fileExtensions = "___mbase";
				
				setGrammarUri("classpath:/org/eclipse/xtext/mbase/Mbase.xtext");
				addReferencedResource(mbaseGenModel);
				setFileExtensions(fileExtensions);
				setGenerateXtendStubs(true);
				addFragment(new GrammarAccessFragment2());
				...
				MbaseGeneratorFragment2 mbase = new MbaseGeneratorFragment2();
				mbase.setGenerateXtendInferrer(false);
				mbase.setUseInferredJvmModel(false);
				mbase.setJdtTypeHierarchy(false);
				addFragment(mbase);
			...
			addLanguage(new XtextGeneratorLanguage() {{
				String fileExtensions = "___mbasewithannotations";
				
				setGrammarUri("classpath:/org/eclipse/xtext/mbase/annotations/MbaseWithAnnotations.xtext");
				addReferencedResource(mbaseGenModel);
				setFileExtensions(fileExtensions);
				setGenerateXtendStubs(true);
				addFragment(new GrammarAccessFragment2());
		...
		Logger.getLogger(GenerateMbase.class).info("Done."); 
	}
