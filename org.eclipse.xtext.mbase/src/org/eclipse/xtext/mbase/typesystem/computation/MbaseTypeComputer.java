 ...
 * 
 * This implementation handles all expression that are defined in mbase.
 * 
 * @author Sebastian Zarnekow - Initial contribution and API
 * @author Moritz Eysholdt - Support for checked exceptions
 */
public class MbaseTypeComputer extends AbstractTypeComputer implements ITypeComputer {
				...
				if (object.getRight() != null) {
					...
					if (earlyExitComputer.isDefiniteEarlyExit(computedType.getExpression())) {
						AbstractDiagnostic diagnostic = new EObjectDiagnosticImpl(
								...
								object,
								MbasePackage.Literals.XVARIABLE_DECLARATION__NAME,
								-1,
								null);
						state.addDiagnostic(diagnostic);
					}
						...
						if (array != null) {
							...
							if (parameterType.isAssignableFrom(arrayComponentType)) {
								...
								AbstractDiagnostic diagnostic = new EObjectDiagnosticImpl(
										...
										object, 
										MbasePackage.Literals.XFOR_LOOP_EXPRESSION__FOR_EXPRESSION, -1, null);
								state.addDiagnostic(diagnostic);
							}
						}
					}
			...
			if (!state.isIgnored(IssueCodes.UNHANDLED_EXCEPTION) && thrownException.isSubtypeOf(Throwable.class)
				...
				if (!declarationFound) {
					...
					state.addDiagnostic(new EObjectDiagnosticImpl(
							...
							object,
							MbasePackage.Literals.XTHROW_EXPRESSION__EXPRESSION,
							-1,
							new String[] { 
