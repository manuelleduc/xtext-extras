...
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XAdditiveExpressionElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XAndExpressionElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XAssignmentElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XConstructorCallElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XEqualityExpressionElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XFeatureCallElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XMemberFeatureCallElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XMultiplicativeExpressionElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XOrExpressionElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XOtherOperatorExpressionElements;
import org.eclipse.xtext.mbase.services.MbaseGrammarAccess.XRelationalExpressionElements;

import com.google.common.collect.Sets;
import com.google.inject.Inject;

public class MbaseSemanticSequencer extends AbstractMbaseSemanticSequencer {
	...
	protected void sequence_XAdditiveExpression_XAndExpression_XAssignment_XEqualityExpression_XMultiplicativeExpression_XOrExpression_XOtherOperatorExpression_XRelationalExpression(ISerializationContext context, XBinaryOperation operation) {
		...
		if (feature.eIsProxy()) {
			List<INode> ops = NodeModelUtils.findNodesForFeature(operation, MbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE);
			for (INode o : ops)
				operatorNames.add(NodeModelUtils.getTokenText(o));
		} else {
			IScope scope = scopeProvider.getScope(operation, MbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE);
			for (IEObjectDescription desc : scope.getElements(feature))
				operatorNames.add(qualifiedNameConverter.toString(desc.getName()));
		}
		ICompositeNode featureNode = (ICompositeNode) nodes.getNodeForSingelValue(MbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, operation.getFeature());
		String featureToken;
	...
	protected void sequence_XFeatureCall(ISerializationContext context, XFeatureCall featureCall) {
		...
		if (featureCall.isExplicitOperationCallOrBuilderSyntax()) {
			...
			if (!arguments.isEmpty()) {
				if (featureCall.isExplicitOperationCall() && isXShortClosureAndBuilderSyntax(arguments, MbasePackage.Literals.XFEATURE_CALL__FEATURE_CALL_ARGUMENTS, nodes)) {
				...
				} else if (featureCall.isExplicitOperationCall() && isXShortClosure(featureCall, MbasePackage.Literals.XFEATURE_CALL__FEATURE_CALL_ARGUMENTS, nodes)) {
					acceptor.accept(featureCallElements.getFeatureCallArgumentsXShortClosureParserRuleCall_3_1_0_0(), arguments.get(0), 0);
				} else {
					int diff = 0;
					if (isBuilderSyntax(featureCall, MbasePackage.Literals.XFEATURE_CALL__FEATURE_CALL_ARGUMENTS, featureCall.isExplicitOperationCall(), nodes)) {
						diff = 1;
					}
					if (featureCall.isExplicitOperationCall()) {
	...
	protected void sequence_XMemberFeatureCall(ISerializationContext context, XMemberFeatureCall featureCall) {
		...
		if (featureCall.isExplicitOperationCallOrBuilderSyntax()) {
			...
			if (!arguments.isEmpty()) {
				...
				} else if (featureCall.isExplicitOperationCall() && isXShortClosure(featureCall, MbasePackage.Literals.XMEMBER_FEATURE_CALL__MEMBER_CALL_ARGUMENTS, nodes)) {
					acceptor.accept(memberFeatureCallElements.getMemberCallArgumentsXShortClosureParserRuleCall_1_1_3_1_0_0(), arguments.get(0), 0);
				} else {
					int diff = 0;
					if (isBuilderSyntax(featureCall, MbasePackage.Literals.XMEMBER_FEATURE_CALL__MEMBER_CALL_ARGUMENTS, featureCall.isExplicitOperationCall(), nodes)) {
						diff = 1;
					}
					if (featureCall.isExplicitOperationCall()) {
						if (arguments.size() - diff > 0)
	...
	protected void sequence_XConstructorCall(ISerializationContext context, XConstructorCall constructorCall) {
		...
		if (!arguments.isEmpty()) {
			if (constructorCall.isExplicitConstructorCall() && isXShortClosureAndBuilderSyntax(arguments, MbasePackage.Literals.XCONSTRUCTOR_CALL__ARGUMENTS, nodes)) {
			...
			} else if (constructorCall.isExplicitConstructorCall() && isXShortClosure(constructorCall, MbasePackage.Literals.XCONSTRUCTOR_CALL__ARGUMENTS, nodes)) {
				acceptor.accept(constructorCallElements.getArgumentsXShortClosureParserRuleCall_4_1_0_0(), arguments.get(0), 0);
			} else {
				int diff = 0;
				if (isBuilderSyntax(arguments, MbasePackage.Literals.XCONSTRUCTOR_CALL__ARGUMENTS, nodes)) {
					diff = 1;
				}
				if (constructorCall.isExplicitConstructorCall()) {
					if (arguments.size() - diff > 0)
