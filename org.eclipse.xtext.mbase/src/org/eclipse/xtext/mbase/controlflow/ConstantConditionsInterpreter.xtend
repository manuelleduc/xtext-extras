...
class ConstantConditionsInterpreter {
	...
	def private boolean isFrommbaseLibrary(XAbstractFeatureCall it, EvaluationContext context) {
		val feature = getFeature(it, context)
		switch feature {
			JvmMember: feature?.declaringType?.packageName == XImportSectionNamespaceScopeProvider.mbase_LIB.toString
			default: false
		}
	}
	
	def dispatch EvaluationResult internalEvaluate(XUnaryOperation it, EvaluationContext context) {
		if (isFrommbaseLibrary(context)) {
	...
	def dispatch EvaluationResult internalEvaluate(XBinaryOperation it, EvaluationContext context) {
		if (isFrommbaseLibrary(context) && rightOperand !== null) {
